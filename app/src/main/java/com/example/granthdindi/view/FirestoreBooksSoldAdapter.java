...
public class FirestoreBooksSoldAdapter extends FirestoreRecyclerAdapter<Book, FirestoreBooksSoldAdapter.BooksViewHolder> {
    private Context context;

    public FirestoreBooksSoldAdapter(@NonNull FirestoreRecyclerOptions<Book> options, Context context) {
        super(options);
    ...
    protected void onBindViewHolder(@NonNull BooksViewHolder holder, int position, @NonNull Book model) {
        ...
        holder.textViewBookStock.setText("Stock: " + String.valueOf(model.getQuantity()));
        holder.textViewBookPrice.setText("₹ " + String.valueOf(model.getPrice()));
    }

    @NonNull
    @Override
    public FirestoreBooksSoldAdapter.BooksViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
